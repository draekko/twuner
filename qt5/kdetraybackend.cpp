...
KDETrayBackend::KDETrayBackend(MainWindow* parent):
    ...
    TrayIconInterface(),
    m_mainWindow(parent),
    m_statusNotifierItem(new KStatusNotifierItem("twuner_qt", this))
{
    m_statusNotifierItem->setAssociatedWidget(parent);
    m_statusNotifierItem->setIconByName("twuner_qt-inactive");
    m_statusNotifierItem->setToolTip("twuner", i18n("Twuner"), "");
    ...
    m_statusNotifierItem->setToolTipTitle(i18n("Twuner"));
...
void KDETrayBackend::showMessage(QString type, QString title, QString message, QString image)
{
    Q_UNUSED(type)
    Q_UNUSED(image)
    m_statusNotifierItem->showMessage(title, message, "twuner", 4000);
}
...
void KDETrayBackend::unreadAlert(QString number)
{
    int n = number.toInt();
    if (n > 0) {
        m_statusNotifierItem->setIconByName("twuner_qt-active");
    } else
        m_statusNotifierItem->setIconByName("twuner_qt-inactive");
    m_statusNotifierItem->setToolTip("twuner", i18n("Twuner"), i18np("1 unread post", "%1 unread posts", n));
}
