    ...
    m_firstLoad(true)
{
    ...
    chdir(PREFIX);
#endif
    setWindowTitle(i18n("Twuner"));
    setWindowIcon(QIcon::fromTheme("twuner_qt", QIcon("share/twuner/image/ic64_twuner.png")));
    qApp->setWindowIcon(QIcon::fromTheme("twuner_qt", QIcon("share/twuner/image/ic64_twuner.png")));
#ifndef MEEGO_EDITION_HARMATTAN
...
#ifndef MEEGO_EDITION_HARMATTAN
    QSettings settings("twuner-qt", "twuner");
    ...
    m_page = new TwunerWebPage(this);
#ifdef HAVE_KDE
    m_page->setNetworkAccessManager(new KIO::Integration::AccessManager(m_page));
#endif

#ifdef Q_OS_UNIX
    QDir dir(QDir::homePath().append("/.config/twuner-qt"));
#else
    QDir dir(QDesktopServices::storageLocation(QDesktopServices::DataLocation).append("/Twuner"));
#endif
...
#ifdef Q_OS_UNIX
    m_webView->load(QUrl("file://" PREFIX "/share/twuner/index.html"));
#else
    QFileInfo f("share/twuner/index.html");
...
void MainWindow::loadFinished(bool ok)
{
    ...
    if (ok) {
        ...
        m_webView->page()->currentFrame()->evaluateJavaScript(
            "overlay_variables(twuner_qt_variables);"
