...
function TwitterClient() {
    ...
    self.default_error_handler = function default_error_handler(url, xhr, textStatus, errorThrown) {
        ...
        case 'notify':
            if (xhr.status !== 0) {
                twuner_notify('Ooops, An Error Occurred!', msg + '\n' + tech_info, null, 'content');
            } else {
        ...
        twuner_log('Error #' + xhr.status + ',' + xhr.statusText, msg + ' ' + url);
        return;
    };
    ...
    self.watch_user_streams = function watch_user_streams(callback) {
        ...
        twuner_log('Streams Open', removeTokensFromUrl(url));

        var xhr = new XMLHttpRequest();
        watch_user_streams.xhr = xhr;
        xhr.open('GET', url, true);
        xhr.setRequestHeader('X-User-Agent', 'Twuner');
        try {
            xhr.setRequestHeader('User-Agent', 'Twuner');
        } catch (e) {
        }
        xhr.createAt = new Date().toLocaleString();
        xhr.onabort = xhr.onerror = function() {
            if (xhr.status == 401 || xhr.status == 407) {
                twuner_log('Streams XHR', 'OAuth error');
                watch_user_streams.disable = true;
            }
            if (xhr.status == 420) {
                twuner_log('Streams XHR', '420 error');
                watch_user_streams.is_running = false;
            }
            watch_user_streams.is_running = false;
            twuner_log('Streams Exit', xhr.createAt + ' -> ' + new Date().toLocaleString());
        }
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 2 && xhr.status === 200) {
                twuner_log('Streams Start', 'Connected');
                ...
                if (xhr.responseText.length > 500000) {
                    twuner_log('Streams Rec', xhr.responseText.length);
                    setTimeout(function() {
                        xhr.abort();
                    },
                    100);
                ...
                if (empty_tester.test(newText)) {
                    twuner_log('Streams XHR', 'Got nothing useful');
                    return;
                }
                if (callback) {
                        ...
                        for (var j = 0; j < line.length; j += 1) {
                            if (!empty_tester.test(line[j])) {
                                try {
                                    ...
                                    // twuner_log('Streams XHR', e.message + '\n' + line);
                ...
                twuner_log('Streams End', 'Connection completed');
            }
    ...
    self.add_streaming_filter = function add_streaming_filter(filter, callback) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 3) {
                ...
                if (obj) {
                    twuner_log('Streaming', obj.id_str + "," + obj.user.screen_name + ":" + obj.text)
                }
            }
        }
        params = [];
