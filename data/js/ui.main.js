...
function load_tweet_success(self, json) {
    ...
    twuner_log('incoming_num of '+self.name, self.incoming_num);
    ...
    if (ui.Main.views[self.name].use_notify) {
        ...
        for (var i = 0; i < self.incoming_num && i <= 3; i += 1) {
            ...
            if (user.screen_name == globals.myself.screen_name)
                continue;
            text = json[i].text;
            twuner_notify(user.screen_name, text, user.profile_image_url , 'content');
            notify_count += 1;
        }
        if (3 < notify_count) {
            twuner_notify("Update page " + self.name
    ...
    if (util.is_native_platform()) {
        twuner_action('system/incoming/' + self.name + '/'
 ...
 *   'retweet_id' which should be used to handle retweeted tweets by Twuner.
 *
...
function bind_tweet_action(id) {
    ...
    $(id).find('a[full_text_id]').unbind().click(function (ev) {
        ...
        globals.network.do_request('GET', 
            'http://twuner.in/tweet/'+full_text_id+'.json', 
            {}, {}, null,
            function (result) {
