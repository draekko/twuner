...
function change_theme(theme_name, theme_path) {
    $('#twuner_theme').attr('href', theme_path + '/style.css');
    $.getJSON(theme_path + '/info.json',
    function (twuner_theme_info) {
        $('#prefs_theme_name').text(twuner_theme_info.name);
        $('#prefs_theme_author').text(twuner_theme_info.author);
        $('#prefs_theme_web').text(twuner_theme_info.web)
            .attr('href', twuner_theme_info.web);
        $('#prefs_theme_desc').text(twuner_theme_info.description);
...
function update_status(text) {
    globals.twitterClient.update_status(text, null,
        function (result) {
            twuner_notify('Notice', 'Sent a status:' + text, null , 'content');
        },
...
function reply_tweet(tid, text) {
    globals.twitterClient.update_status(text, tid,
        function (result) {
            twuner_notify('Notice', 'Reply a status:' + text, null , 'content');
        },
        function (xhr, textStatus, errorThrown) {
        });
}
...
function twuner_notify(title, summary, image, type) {
    notification.push(title, summary, image, type);
}

function twuner_log(label, content) {
    if (conf.settings.use_verbose_mode) {
        if (util.is_native_platform()) {
            twuner_action('action/log/' + encodeURIComponent(label)
...
function unread_alert(count) {
    ...
    var proc = function (){
        sp ^= 1;
        document.title = '('+(sp?globals.unread_count:' ! ')+')' + _('twuner') + ' | ' + conf.current_name;
        globals.unread_alert_timer = setTimeout(proc, 2000);
    }
    clearTimeout(globals.unread_alert_timer);
    if (count == 0) {
        globals.unread_count = 0;
        document.title = _('twuner') + ' | ' + conf.current_name;
        if (util.is_native_platform()) {
            twuner_action('system/unread_alert/0');
        }
    } else {
        globals.unread_count += count;
        if (util.is_native_platform()) {
            twuner_action('system/unread_alert/'+globals.unread_count);
        } else {
            proc();
        }
    }
...
function overlay_variables(vars) {
    ...
    twuner_log('init', 'overlay_variables()');
    if (util.is_native_platform()) {
        // native variables
...
function on_load_finish() {
    ...
    if (util.is_native_platform() && globals.load_flags == 0) {
        ...
        twuner_log('init', 'on_load_finish()');
        globals.load_flags = 1;
        // 1. load builtins & extra extensions
        var procs = [];
        procs.push(function () {
            twuner_log('init', 'on_load_finish() -> ext.load_builtin_exts();');
        ...
        if (util.is_native_platform()) {
            procs.push(function() {
                twuner_log('init', 'on_load_finish() -> ext.load_exts();');
        ...
        procs.push(function () {
            twuner_log('init', 'on_load_finish() -> ext.init_exts();');
            ext.init_exts();
            $(window).dequeue('_on_load_finish');
        });
        ...
        if (util.is_native_platform()) {
            procs.push(function () {
                twuner_log('init', 'on_load_finish() -> push settings');
                twuner_action('system/load_settings/'
        ...
        // 3. i18n
        procs.push(function() {
            twuner_log('init', 'on_load_finish() -> i18n.init()');
            i18n.init(function () {
                $(window).dequeue('_on_load_finish');
            });
        });
        // 4. init_ui();
        procs.push(function() {
            twuner_log('init', 'on_load_finish() -> init_ui()');
            init_ui();
            $(window).dequeue('_on_load_finish');
        });
