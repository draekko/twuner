...
function search_image_with_google(li_id) {
    ...
    twuner_log('Imghp', 'img_num: '+media_array.length);
    if (media_array.length == 0) {
        ...
        $.each(
            ...
            function(i,e){
                ...
                twuner_log('Imghp', 'search_url: '+search_img_url);
            }
        );
        return;
    }
...
enable:
function enable() {
    ext.add_tweet_more_menuitem('ext_btn_twuner_imghp'
        , 'Google Image'
        , true
...
disable:
function disable() {
    ext.remove_tweet_more_menuitem('ext_btn_twuner_imghp');
}
