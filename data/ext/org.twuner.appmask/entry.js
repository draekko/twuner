...
url: 'https://github.com/xnreformer/twuner-exts',
...
function open_option_dialog() {
	...
	var body = '<style>'+
			'#ext_twuner_appmask_opt_dialog .dialog_body>ol{list-style-type:decimal;margin-left:25px}'+
			'#ext_twuner_appmask_opt_dialog .dialog_body li{margin:10px 0}'+
			'#ext_twuner_appmask_opt_dialog .dialog_body table{margin:0 10px}'+
			'#ext_twuner_appmask_opt_dialog .dialog_body td{border:none;padding:0px}'+
			'#ext_twuner_appmask_opt_dialog .dialog_body input{width:200px}'+
		'</style>'+
		'<ol>'+
		'<li>'+
		...
		'<tr>'+
		'<td>Consumer Key:</td>'+
		'<td><input type="text" id="ext_twuner_appmask_consumer_key"/></td>'+
		'</tr>'+
		'<tr>'+
		'<td>Consumer Secret:</td>'+
		'<td><input type="text" id="ext_twuner_appmask_consumer_secret"/></td>'+
		'</tr>'+
		'</tbody>'+
		'</table>'+
		'</li>'+
	...
	ext.AppMask.option_dialog = widget.DialogManager.build_dialog(
		'#ext_twuner_appmask_opt_dialog', title, '', body, [
	...
	for (var key in prefs) {
		$('#ext_twuner_appmask_' + key).val(prefs[key]);
	}
...
function on_btn_save_prefs_clicked() {
	...
	for (var key in prefs) {
		var value = $('#ext_twuner_appmask_' + key).val();
		if (value) {
			prefs[key] = value;
		} else {
			toast.set("Please fill the " + key.replace("_", " ") + " field").show();
			$('#ext_twuner_appmask_' + key).focus();
			return;
		}
	}
